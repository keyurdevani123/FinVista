...
export function BudgetProgress({ initialBudget, currentExpenses }) {
  ...
  const overAmount = isOverBudget
    ? (currentExpenses - initialBudget.amount).toFixed(2)
    : 0;
        ...
        <div className="flex justify-between items-center">
          <div className="space-y-1">
            ...
            {initialBudget && !isEditing && (
              <CardDescription className={`text-sm ${isOverBudget ? "text-red-500 font-medium" : ""}`}>
                ₹{currentExpenses.toFixed(2)} of ₹{initialBudget.amount.toFixed(2)} spent
                {isOverBudget && ` — ₹${overAmount} over limit`}
              </CardDescription>
            )}
          </div>
        ...
        {initialBudget && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <p className={`text-xs font-medium ${isOverBudget ? "text-red-500" : "text-muted-foreground"}`}>
                {isOverBudget ? (
